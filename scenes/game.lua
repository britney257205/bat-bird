...
local function addPipe()
    ...
    local pTop = display.newImageRect(grpWorld, "pipe.png", 90, 600)
    pTop.x = _W + 50
    pTop.y = yPosition - (distanceBetween / 2) - 350
    pTop.type = "pipe"
    pTop.xScale = -1
    pTop.rotation = -180
    pipes[#pipes+1] = pTop

    local pBottom = display.newImageRect(grpWorld, "pipe.png", 90, 600)
    pBottom.x = _W + 50
    pBottom.y = yPosition + (distanceBetween / 2) + 350
    pBottom.type = "pipe"
...
function scene:create( event )
    ...
    bird = display.newImageRect( grpWorld, "flappy.png", 50, 40 )
    bird.x = 100
    bird.y = _CY
    bird.velocity = 0
    bird.crashed = false
    ...
    lblScore = display.newText("0p", _CX, 180, "Pumpkin Pancakes.ttf", 40)
